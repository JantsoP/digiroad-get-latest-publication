...
onlineversion=$(curl -s -I  https://aineistot.vayla.fi/digiroad/latest/KokoSuomi_DIGIROAD_R_EUREF-FIN.zip | awk '/Last-Modified/{ date=""; for(i=2;i<=NF;++i) date=(date " " $i); print date;}' | xargs -I{} date -d {} +"%s")
...
if [ $onlineversion -gt $offlineversion ]
        then
            ...
            curl -o /opt/KokoSuomi_DIGIROAD_R_EUREF-FIN.zip https://aineistot.vayla.fi/digiroad/latest/KokoSuomi_DIGIROAD_R_EUREF-FIN.zip
